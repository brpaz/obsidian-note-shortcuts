...
3. That publish fires a `release: published` event, which two `ci.yml` jobs react to in parallel: `attach-release-assets` builds the plugin (bumping its own in-runner `manifest.json` copy first, since the release asset — not the repo-root file — is what BRAT/Obsidian read) and uploads `main.js`/`manifest.json`; `sync-release-metadata` bumps the repo-root `manifest.json`/`versions.json` and mirrors the release notes into `CHANGELOG.md`, landing both as one `chore(release): sync repo to vX.Y.Z` commit on `main` rather than two separate bot commits.
4. `lint` (build + eslint) runs unconditionally on every push, PR, and release — not gated on event type.
